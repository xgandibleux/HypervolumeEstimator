#= EXPERIMENT 2.2: 
  ...
  - report for each instance 
    - cardinality of Y_PN
    - elapsed time for computing Y_PN   
    - H measured     
...
open(instanceName*".res", "w") do ioAll
    ...
    # =============================================================================
    println("\nCompute S, a representative set of nondominated points...")
    start = time()
    S, cardS = approxMO01UKP(p, w, c)
    save_nondominatedpoints(instanceName*".ypn",S)
    ...
    @assert 2 <= o <= 6 "Error: Number of objectives must be between 2 and 6"
