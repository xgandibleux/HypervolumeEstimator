...
open(instanceName*".res", "w") do ioAll
    ...
    for instanceID in 1:nInstances
        ...
        for iWeight in 1:length(listnbrWeights)
            ...
            relative_error_Hestimated = (Float64)[]

            for _ in 1:trials
                startH = time()
             #   H̃, numberOfWeights, info_for_CI, listL_weighted = Hrevised2(solver, p,w,c, rp, nbrWeights)
                H̃, numberOfWeights, info_for_CI = Hrevised3(Hmeasure, solver, p,w,c, rp, nbrWeights)                
                t_elapsedH = round(time() - startH, digits=2)



                # compute relative error on listL_weighted for 1 trial
                #relative_error_listL_weighted = abs.( (Hmeasure .- listL_weighted) ./ Hmeasure) 
               # relative_error_Hestimated = (Float64)[]
                #for i in 1:length(listL_weighted)
                #    push!(relative_error_listL_weighted, abs( (Hmeasure - listL_weighted[i]) / Hmeasure) )
                    push!(relative_error_Hestimated, abs( (1 - info_for_CI[1]) / 1) )                    
                #end
        ...
        #        @printf(" %.5f ", round(info_for_CI[1], digits=5) )
        ...
        #        @printf(" [%.5f, %.5f] \n", round(info_for_CI[2], digits=5), round(info_for_CI[3], digits=5) )
            ...
            println("  H estimated with ", listnbrWeights[iWeight])#  $numberOfWeights weight ")

            # compute average value of relative_error_listL_weighted
            average_relative_error_Hestimated = average_value(relative_error_Hestimated)
            push!(All_average_relative_error_Hestimated, average_relative_error_Hestimated)            

            print("\n  !!! Average relative error Hestimated : ")
            println(average_relative_error_Hestimated)
            print("\n  !!! All average relative error Hestimated : ")
            println(All_average_relative_error_Hestimated)
...
#listnbrWeights = [100,500,1000,1500,2000,5000,10000]
plot(listnbrWeights, All_average_relative_error_Hestimated,# allareH̃, 
    seriestype = :line, 
    marker = :circle,
