...
ψ(ukp, rng) = (ϕ = abs.(randn(rng, ukp.o)); ϕ / norm(ϕ))
...
function L_threaded(ukp, rp, N)
    ...
    Threads.@threads for chunk in chunks
        ...
        @expression(m, z[k=1:ukp.o], sum(ukp.p[k,j] * x[j] for j in 1:ukp.n))
        @variable(m, α ≥ 0)
        @objective(m, Max, α)

        for i in chunk
            λ_ψ = λ(ψ(ukp, rng))
            @constraint(m, con[k=1:ukp.o], α ≤ -λ_ψ[k] * (rp[k] - z[k]))
            optimize!(m)
            @assert is_solved_and_feasible(m) "STOP: optimal solution not found" 
            listL[i] = objective_value(m)
            for k in 1:ukp.o; delete(m, con[k]); end
            unregister(m, :con)
        end
...
function H(ukp, rp, N, Hexact)
    ...
    coeff   = 1/ukp.o * (2 * π^(ukp.o/2)) / (gamma(ukp.o/2) * 2^ukp.o)
    H_est   = coeff * sum(l^ukp.o for l in list_L) / N
    lw_norm = coeff / Hexact .* [l^ukp.o for l in list_L]
...
function main()
    ...
    println("Instance : $(ukp.n) items, $(ukp.o) objectives, capacity = $(ukp.c)")
